...
#GLOBAL SETTINGS
homedir = '/home/zanetti/test2-2/' #people_automatic_pipeline_MF_nonpeople/' #people_automatic_pipeline_people_nonpeople/' #people_nonpeople4/' #people_automatic20000_extended/' #people_automatic9000_cleanup_extended/' #M_F_nonpeople/' #people_automatic2100_cleanup2/'   # ##MIT_dataset_even/'#3fold_Full_set1/'#Main directory of dataset
architecture_type = 'CNN4' #Possibilities: MLP, CNN2, CNN3 ,ResNet1. Standard use: CNN2
png_conversion = 1 #Set to 1 for image preprocessing and png conversion. To 0 if done before.
num_epochs_ = 0 #Times all images are forward-propagated.
batch_size = 16
image_size_h = 64 #Images are re-sized to image_size_h x image_size_w.
image_size_w = 32
crop_size_h = 64 #Images are re-sized to crop_size_h x crop_size_w.
crop_size_w = 32
c_2_h = crop_size_h / 2 #Leave like this for central cropping.
c_2_w = crop_size_w / 2
num_of_classes = 2 #The program will load the first num_of_classes image folders for training.
learning_rate_ = 0.001 #Kepp between 0.001 and 0.020
momentum_ = 0.9 #Keep close to 0.9
toLoadWeights = True
 #To resume training from previous state.
weights_name = 'weights_people_automatic2100_cleanup2_ep._20_acc._80.2419354839' #'weights_people_automatic2100_cleanup2_ep._20_acc._80.2419354839' #all entire file name without .npz, name of weights to load
save_weights = 1 #at 1 to save weights each epoch.
simulation_name = 'nonimporta'#'43cl._1ite_segm'#'std_43cl_10ite_32x32_halveall_fcl256' #To be changed every time architecture or data changes to save the weights. 
...
layer_n_filters_arr = [1] #layer number of layers for which weights should be displayed.
show_confusion_matrix = 1 #at 1 to show confusion matrix.
eval_cat_cross = 0 #do the testing also with categorical crossentropy.
eval_sq_err = 0 #do the testing also with squared error.
do_only_testing = 0 #at 1 to skip training.
version = '430.56'
savepeople = 1 #at 1 to 
...
def prep_image_for_test_A(img):
    ...
    im = skimage.transform.resize(im, (image_size_h, image_size_w))
    #if h < w:
    #    im = skimage.transform.resize(im, (image_size_h, w*image_size_w/h))
    #else:
    #    im = skimage.transform.resize(im, (h*image_size_h/w, image_size_w))
 
    h, w, _ = im.shape
    #print(str(h) + str(w))
    im = im[h//2-c_2_h:h//2+c_2_h, w//2-c_2_w:w//2+c_2_w]
...
def prep_image_for_test_B(img, for_test): 
    ...
    for i in range (0,crop_size_h):
       for j in range (0,crop_size_w):                      
               
               #currval = float32(255*255*(0.2990*im[i,j,0] + 0.5870*im[i,j,1] + 0.1440*im[i,j,2]))              
               im[i,j,0] = im[i,j,0]# 255*255*
               im[i,j,1] = im[i,j,1]
               im[i,j,2] = im[i,j,2]
...
def load_dataset():    
    ...
    def load_train_images():
        ...
        for classes in range(0,num_of_classes): 
           ...
           if png_conversion == 1:          
             for file in glob.glob(homedir + str(classes) + '/' + str(classes) + '_train/*.jpg'):
                 #print('classes index:' + str(classes)) 
                 #print('TEST DIRECTORY' + homedir + str(classes) + '_train/*.jpg')
    ...
    def load_test_images():
        ...
        for classes in range(0,num_of_classes):          
           if png_conversion == 1:
             for file in glob.glob(homedir + str(classes) + '/' + str(classes) + '_test/*.jpg'):
                 im_temp = Image.open(file)                 
                 file = file[:-4]
                 im_temp.save(file + '.png')
...
def build_ResNet1(input_var=None):
    ...
    l_in = lasagne.layers.InputLayer(shape=(None, 3, crop_size_h, crop_size_w),
                                        input_var=input_var)
...
def build_cnn1(input_var=None):
    print("building CNN1...")

    # Input layer, as usual:
    #0
    network[0] = lasagne.layers.InputLayer(shape=(None, 3, crop_size_h, crop_size_w),
                                        input_var=input_var)   
    #1
    network[1] = lasagne.layers.Conv2DLayer(
              network[0], num_filters=32, filter_size=(3, 3), stride = 1, pad = 0,
    ...
    network[3] = lasagne.layers.Conv2DLayer(
              network[2], num_filters=64, filter_size=(3,3), pad = 0,
              nonlinearity=lasagne.nonlinearities.rectify,
              W=lasagne.init.GlorotNormal(),b=lasagne.init.Uniform())
    #4
    ...
    #5
    network[5] = lasagne.layers.LocalResponseNormalization2DLayer(network[3], alpha=0.0001)        
    
    #6
    network[6] = lasagne.layers.Conv2DLayer(#512
              network[4], num_filters=128, filter_size=(3,3), pad=0, 
              nonlinearity=lasagne.nonlinearities.rectify,
              W=lasagne.init.GlorotNormal(),b=lasagne.init.Uniform())
              
    #7
    network[7] = lasagne.layers.Conv2DLayer(
              network[5], num_filters=256, filter_size=3, pad=0, 
              nonlinearity=lasagne.nonlinearities.rectify,
              W=lasagne.init.GlorotNormal(),b=lasagne.init.Uniform())

    #8
    network[8] = lasagne.layers.MaxPool2DLayer(network[6], pool_size=2, stride=2, ignore_border=False)

    #9
    network[9] = lasagne.layers.LocalResponseNormalization2DLayer(network[7], alpha=0.0001) 

    #10
    network[10] = DropoutLayer(network[8], p=0.25)
    ...
    #12          
    network[12] = DropoutLayer(network[10], p=0.25)          
...
def build_cnn3(input_var=None):
    ...
    #
    network[1] = lasagne.layers.Conv2DLayer(
              network[0], num_filters=100, filter_size=(5, 5), stride = 1, pad = 0,
    ...
    #
    network[2] = lasagne.layers.MaxPool2DLayer(network[1], pool_size=2, stride=2, ignore_border=False)
    
    #network[5] = lasagne.layers.LocalResponseNormalization2DLayer(network[4], alpha=0.0001)        
    
    #
    network[3] = lasagne.layers.Conv2DLayer(#512
              network[2], num_filters=200, filter_size=(5,5), pad=0, 
              nonlinearity=lasagne.nonlinearities.tanh,
              W=lasagne.init.GlorotNormal(),b=lasagne.init.Uniform())
    ...
    #
    network[5] = lasagne.layers.DenseLayer(
              network[4], num_units=100,  #4096
              nonlinearity=lasagne.nonlinearities.tanh)
    
    #          
    #network[6] = DropoutLayer(network[11], p=0.5)          

    #
    outlayer = lasagne.layers.DenseLayer(
              network[5], num_units=num_of_classes,
              nonlinearity=lasagne.nonlinearities.softmax)
...
def build_cnn4(input_var=None):
    ...
    network[5] = lasagne.layers.Conv2DLayer(#512
              network[4], num_filters=200, filter_size=(3,3), pad=0, 
              nonlinearity=lasagne.nonlinearities.tanh,
              W=lasagne.init.GlorotNormal(),b=lasagne.init.Uniform())
    #
    #network[5] = lasagne.layers.LocalResponseNormalization2DLayer(network[4], alpha=0.0001) 
    ...
    #
    network[6] = lasagne.layers.DenseLayer(
              network[5], num_units=100,  #4096
              nonlinearity=lasagne.nonlinearities.tanh)
    
    #          
    network[7] = DropoutLayer(network[6], p=0.5)          

    #
    outlayer = lasagne.layers.DenseLayer(
              network[7], num_units=num_of_classes,
              nonlinearity=lasagne.nonlinearities.softmax)
...
def build_model():
    ...
    net['loss3/classifier'] = DenseLayer(net['pool5/7x7_s1'],
                                         num_units=1000,
                                         nonlinearity=linear)
    outlayer = NonlinearityLayer(net['loss3/classifier'], #net['prob']
...
def main(model='fede', num_epochs= num_epochs_):
    ...
    X_test = [] 
    y_test = []
    X_train, y_train, X_test, y_test, z_test = load_dataset()
    ...
    if architecture_type == 'MLP': architecture, network = build_mpl(input_var)
    if architecture_type == 'CNN1': architecture, network = build_cnn1(input_var)
    ...
    if architecture_type == 'CNN4': architecture, network = build_cnn4(input_var)
    if architecture_type == 'ResNet1': architecture, network = build_ResNet1(input_var)
    ...
    if (toLoadWeights is True):
        ...
        try:
            with np.load(weights_name + '.npz') as f:
    ...
    for epoch in range(num_epochs): 
        ...
        for batch in iterate_minibatches(X_train, y_train, batch_size, shuffle=True): 
            batch_ite = batch_ite + 1
            if (1000 % batch_ite == 0):
                 print("epoch %d, minibatch %d" %(epoch+1, batch_ite))
        ...
        this_epoch_accuracy = val_acc / val_batches * 100
        #file.write("epoch %f: tl %f , vl %f , val.acc %f -" %(epoch, train_err / train_batches , val_err / val_batches , this_epoch_accuracy))
        #file.close()    

        if save_weights == 1:
           np.savez('weights_' + simulation_name + '_ep._' + str(epoch) + '_acc._' + str(this_epoch_accuracy) + '.npz', *lasagne.layers.get_all_param_values(network))
           print("weights saved")
...
############################################################################## 
    ...
    if show_confusion_matrix == 1:
       ...
       confusion_array = zeros((num_of_classes,num_of_classes), float) #should be num_of_classes
       for i in range(0,len(X_test)):#len(X_test)):  #(start,end,step)
           ...
           if decision != y_test[i]:            
              if (os.path.isdir('errors_' + simulation_name )):
           ...
           if savepeople == 1:
              if y_test[i] == 1: 
                 if decision == 1:
                    if (os.path.isdir(homedir + '1to1/' )):
                       scipy.misc.imsave(homedir + '1to1/' + z_test[i] +  '.png', test_images2[i])  
                       print('saved1')                 

           if savepeople == 1:
              if y_test[i] == 1: 
                 if decision == 0:
                    if (os.path.isdir(homedir + '1to0/' )):
                       scipy.misc.imsave(homedir + '1to0/' + z_test[i] + '.png', test_images2[i] )
                       print('saved2')   

           if savepeople == 1:
              if y_test[i] == 0: 
                 if decision == 0:
                    if (os.path.isdir(homedir + '0to0/' )):                      
                       scipy.misc.imsave(homedir + '0to0/' + z_test[i] +  '.png', test_images2[i] )
                       print('saved3')

           if savepeople == 1:
              if y_test[i] == 0: 
                 if decision == 1:
                    if (os.path.isdir(homedir + '0to1/' )):
                       scipy.misc.imsave(homedir + '0to1/' + z_test[i] + '.png', test_images2[i] )
                       print('saved4')

           if savepeople == 1:
              if y_test[i] == 0: 
                 if decision == 2:
                    if (os.path.isdir(homedir + '0to2/' )):
                       scipy.misc.imsave(homedir + '0to2/' + z_test[i] + '.png', test_images2[i] )
                       print('saved5')

           if savepeople == 1:
              if y_test[i] == 2: 
                 if decision == 0:
                    if (os.path.isdir(homedir + '2to0/' )):
                       scipy.misc.imsave(homedir + '2to0/' + z_test[i] + '.png', test_images2[i] )
                       print('saved6')

           if savepeople == 1:
              if y_test[i] == 2: 
                 if decision == 1:
                    if (os.path.isdir(homedir + '2to1/' )):
                       scipy.misc.imsave(homedir + '2to1/' + z_test[i] + '.png', test_images2[i] )
                       print('saved7')

           if savepeople == 1:
              if y_test[i] == 1: 
                 if decision == 2:
                    if (os.path.isdir(homedir + '1to2/' )):
                       scipy.misc.imsave(homedir + '1to2/' + z_test[i] + '.png', test_images2[i] )
                       print('saved8')

           if savepeople == 1:
              if y_test[i] == 2: 
                 if decision == 2:
                    if (os.path.isdir(homedir + '2to2/' )):
                       scipy.misc.imsave(homedir + '2to2/' + z_test[i] + '.png', test_images2[i] )
                       print('saved9')
    ...
    if show_features == 1:
       counter = 0
       for file in glob.glob(homedir + "*.jpg"):
    ...
    if show_examples == 1:
       ...
       for file in glob.glob(homedir + "*.png"):
           ...
           im = prep_image_for_test_A(file)
           rawim, im = prep_image_for_test_B(im, 1) 
           #matplotlib.use('Agg') #added for run on remote server
           #plt.imshow(rawim)
           #show()
