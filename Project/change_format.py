...
#GLOBAL SETTINGS
homedir = '/home/zanetti/FINAL_PIPELINE_extended_renamed3/' #M_F_nonpeople/' #people_automatic_pipeline_MF_nonpeople/' #people_automatic_pipeline_people_nonpeople/' #people_automatic20000_extended/' #people_automatic5000/' #people_automatic9000_cleanup_extended/' #people_automatic2100_cleanup2/'#people_automatic2100_cleanup2_plus_MIT/' #people_automatic2100_cleanup2/'#3fold_Full_set1/'#Main directory of dataset
architecture_type = 'CNN2' #Possibilities: MLP, CNN2, CNN3 , CNN4, googlenet, ResNet1. Standard use: CNN2
...
batch_size = 16
image_size_h = 80 #96#80
image_size_w = 40 #48#40
crop_size_h = 64 #Images are re-sized to crop_size x crop_size.
crop_size_w = 32
c_2_h = crop_size_h / 2 #Leave like this for central cropping.
c_2_w = crop_size_w / 2
num_of_classes = 3 #The program will load the first num_of_classes image folders for training.
learning_rate_ = 0.001 #Keep between 0.001 and 0.020
momentum_ = 0.9 #Keep close to 0.9
toLoadWeights = True #To resume training from previous state.
weights_name = 'temp' #without .npz
save_weights = 0
simulation_name = 'temp'#'43cl._1ite_segm'#'std_43cl_10ite_32x32_halveall_fcl256' #To be changed every time architecture or data changes to save the weights. 
        ...
        for classes in range(0,num_of_classes): 
           ...
           if png_conversion == 1:          
             ...
             for file in glob.glob(homedir + str(classes) + '/' + str(classes) + '_train/*.png'):
                 ...
                 scipy.misc.toimage(im_temp).save(file + '.jpg')
...
def build_cnn3(input_var=None):
    ...
    network[6] = lasagne.layers.Conv2DLayer(
              network[4], num_filters=256, filter_size=3, pad=1, 
              nonlinearity=lasagne.nonlinearities.rectify,
              W=lasagne.init.GlorotNormal(),b=lasagne.init.Uniform())
    ...
    #7
    network[8] = lasagne.layers.Conv2DLayer(
              network[7], num_filters=256, filter_size=3, pad=1, 
              nonlinearity=lasagne.nonlinearities.rectify,
              W=lasagne.init.GlorotNormal(),b=lasagne.init.Uniform())

    #8
    network[9] = lasagne.layers.MaxPool2DLayer(network[8], pool_size=2, stride=2, ignore_border=False)
    ...
    #12          
    network[11] = DropoutLayer(network[11], p=0.5)          

    #13
    outlayer = lasagne.layers.DenseLayer(
              network[10], num_units=num_of_classes,
              nonlinearity=lasagne.nonlinearities.softmax)
...
def build_cnn4(input_var=None):
    ...
    #
    #network[4] = lasagne.layers.Conv2DLayer(
    #          network[3], num_filters=150, filter_size=3, pad=0, 
    #          nonlinearity=lasagne.nonlinearities.rectify,
    #          W=lasagne.init.GlorotNormal(),b=lasagne.init.Uniform())

    #network[5] = lasagne.layers.Conv2DLayer(
    #          network[4], num_filters=256, filter_size=3, pad=1, 
    #          nonlinearity=lasagne.nonlinearities.rectify,
    #          W=lasagne.init.GlorotNormal(),b=lasagne.init.Uniform())
    ...
    network[5]  =  lasagne.layers.ElemwiseMergeLayer( [network[2], network[4]], theano.tensor.maximum) #add, mul, maximum
