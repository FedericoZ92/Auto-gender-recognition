...
%                         of individual image sub-regions as person or
%                         non-person during multi-scale object detection.
%                         Increase this threshold in situations where many
%                         false detections occur. Typical values range from
%                         -1 to 2.
%
%                         Default: -1
%
...
[bboxes, scores] = vision.internal.acf.detect(Iroi, detector, params);
...
bboxes = clipBBox(bboxes, size(Iroi));
...
if useROI
    ...
    if ~isempty(userInput.ROI)
        ...
        checkImageSizes(sz, userInput, wasMinSizeSpecified, modelSize, ...
            'vision:detectPeopleACF:ROILessThanMinSize', ...
            'vision:detectPeopleACF:ROILessThanModelSize');
    end
       
else        
    
    checkImageSizes([M N], userInput, wasMinSizeSpecified, modelSize, ...
        'vision:detectPeopleACF:ImageLessThanMinSize', ...
        'vision:detectPeopleACF:ImageLessThanModelSize');
...
% Get model and and extract additional parameters
