...
elseif ispc
result_comment='_with_bootstrap_3';
...
% hardfeatsdir=[root_dir '/Matlab/pedestrian_detection/hardfeats'];
...
%     load([modeldir '/model_' methods '.mat'], 'model');
% end
% FeatsNegHard=bootstrap(imgNegList,nori,border,strideh,stridew,scaleratio,model,nmax_param.th,feats_dir,methods);
% save([feats_dir '/FeatsNegHard_1_' methods '.mat'], 'FeatsNegHard');
% t2=toc;
%%%%%%%%%%%%%%%%%% Retrainning Model%%%%%%%%%%%%%%%%%%%%
...
save([feats_dir 'FeatsNeg_' methods '.mat'], 'FeatsNeg');
model=train_save(FeatsPos, FeatsNeg,1,['bootstrap_1_' methods],modeldir); %1 intersection, 2 chisquared, and 3 JS kernels
t3=toc;
%%%%%%%%%%%%%%%%% Bootstraping 2 %%%%%%%%%%%%%%%%%%%%
tic;
nmax_param.th=0.0;
if(~exist('model','var'))
    load([modeldir 'model_bootstrap_1_' methods '.mat'], 'model');
end
FeatsNegHard=bootstrap(imgNegList,nori,border,strideh,stridew,scaleratio,model,nmax_param.th,feats_dir,methods);
save([feats_dir 'FeatsNegHard_2_' methods '.mat'], 'FeatsNegHard');
t4=toc;
%%%%%%%%%%%%%%%%%% Retrainning Model%%%%%%%%%%%%%%%%%%%%
if(~exist('FeatsNegHard','var'))
    load([feats_dir 'FeatsNegHard_2_' methods '.mat'], 'FeatsNegHard');
end
if(~exist('FeatsNeg','var'))
    load([feats_dir 'FeatsNeg_' methods '.mat'], 'FeatsNeg');
end
...
FeatsNeg=[FeatsNeg;FeatsNegHard];
save([feats_dir 'FeatsNeg_' methods '.mat'], 'FeatsNeg');
model=train_save(FeatsPos, FeatsNeg,1,['bootstrap_2_' methods],modeldir); %1 intersection, 2 chisquared, and 3 JS kernels
t5=toc;
%%%%%%%%%%%%%%%%% Bootstraping 3 %%%%%%%%%%%%%%%%%%%%
tic;
nmax_param.th=0.0;
if(~exist('model','var'))
    load([modeldir '/model_bootstrap_2_' methods '.mat'], 'model');
end
FeatsNegHard=bootstrap(imgNegList,nori,border,strideh,stridew,scaleratio,model,nmax_param.th,feats_dir,methods);
save([feats_dir 'FeatsNegHard_3_' methods '.mat'], 'FeatsNegHard');
t6=toc;
%%%%%%%%%%%%%%%%%% Retrainning Model%%%%%%%%%%%%%%%%%%%%
if(~exist('FeatsNegHard','var'))
    load([feats_dir 'FeatsNegHard_3_' methods '.mat'], 'FeatsNegHard');
end
if(~exist('FeatsNeg','var'))
    load([feats_dir 'FeatsNeg_' methods '.mat'], 'FeatsNeg');
end
...
FeatsNeg=[FeatsNeg;FeatsNegHard];
save([feats_dir 'FeatsNeg_' methods '.mat'], 'FeatsNeg');
model=train_save(FeatsPos, FeatsNeg,1,['bootstrap_3_' methods],modeldir); %1 intersection, 2 chisquared, and 3 JS kernels
t7=toc;
%%%%%%%%%% Compute Approx Model%%%%%%%%%%%%%%%%%%%%
if(~exist('model','var'))
    load([modeldir 'model_bootstrap_3_' methods '.mat'], 'model');
end
% if(~exist('model','var'))
%     load([modeldir '/model_' methods '.mat'], 'model');
% end
...
nmax_param.th = -2.0;
%%%%%%%%%%%%%%%% Benchmark Exact %%%%%%%%%%%%%%%%%%%%
benchmark(imgTestPosList,[result_dir 'det+pos/'],nori,border,window_size,block_sizes,strideh,stridew,scaleratio,offh,offw,model,nmax_param, 0);
benchmark(imgTestNegList,[result_dir 'det+neg/'],nori,border,window_size,block_sizes,strideh,stridew,scaleratio,offh,offw,model,nmax_param, 0);
%%%%%%%%%%%%%%%% Benchmark Approx %%%%%%%%%%%%%%%%%%%%
...
t9=toc;
%%%%%%%%%%%% Helper Function %%%%%%%%%%%%%%%%
function featsum=bootstrap(imList,nori,border,strideh,stridew,scaleratio,model,nmax_param_th,feats_dir,methods)
parfor_progress(length(imList));
% featsum=zeros(60000,2268);
featsum=[];
...
% disp(sprintf('%.2fs to compute features, %.2fs to classify %i features..\n',feature_time, classification_time, size(rawr,1)));
    indx = raws > nmax_param_th;
    feats=feats(indx,:);
    featsum=[featsum;feats];
...
% end
% feat=feat(1:sum,:);
% save([feats_dir '/' methods '_hardfeatures.mat'], 'featsum');
% fprintf('%.2fs to save hardfeatures\n', toc);
end
%%%%%%%%%%%% Helper Function %%%%%%%%%%%%%%%%
function feat=bootstrap_fast(imList,nori,border,window_size,block_sizes,strideh,stridew,scaleratio,offh,offw,model,nmax_param_th,hardfeatsdir,methods)
parfor_progress(length(imList));
parfor i=1:length(imList);
    tic;
    image=imread(imList{i});
    [feats,~,~,~,~] = ...
        compute_features_fast_nopad(image,nori,border,window_size,block_sizes,strideh,stridew,scaleratio,offh,offw);
    feats=double(feats);
%     feature_time=toc;
%     rawr=[win_posw;win_posh;winw;winh]';
    labels=ones(size(feats,1),1); 
